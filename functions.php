...
function idtonames($id,$idname,$name,$table){
  $sql = 'SELECT '.$name.' FROM '.$table.' WHERE '.$idname.' = '.$id.'';
  // echo $sql;
  $row = mysqli_fetch_assoc(mysqli_query(connect_db(),$sql));
  return $row[$name]
  ;
}
function employeOfCompany(){
  $array = [];
  $sql = "SELECT  `attendance_id` FROM `customer_employee_data` WHERE owner_id = ".$_SESSION['id']."";
  $result = mysqli_query(connect_db(),$sql);
  while($row = mysqli_fetch_assoc($result)){
                                                // echo $row['attendance_id'];
    array_push($array, $row['attendance_id']);
  }
                                            // print_r($array);
  return implode(",", $array);
}

function dropDownOption($select,$select2,$from,$condition){
  $conn = connect_db();
  $sql_id = 'SELECT `'.$select.'`,`'.$select2.'` FROM `'.$from.'`'.$condition.''; 
